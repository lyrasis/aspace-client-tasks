module Common
  class Objects < Thor
    ...
    def get_resources
      page = 1
      data = []
      response = Aspace_Client.client.get('resources', query: {page: page})
    ...
    def get_aos
      page = 1
      data = []
      response = Aspace_Client.client.get('archival_objects', query: {page: page})
      last_page = response.result['last_page']
      while page <= last_page
        response = Aspace_Client.client.get('archival_objects', query: {page: page})
        data << response.result['results']
    ...
    desc 'make_index_aos_dynamic COMPONENT_ID_OR_EXTERNAL_ID', 'create the following index - "component_id_or_external_id:uri".'
    long_desc <<~LONGDESC
      @param component_id_or_external_id [String] The ID to use to create the index.
        Two options: component_id and external_id.
        external_id picks the first external_id in the record.
      @param return [Hash] The index.
    LONGDESC
